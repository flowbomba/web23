...
switch ($action) {
	...
	case 'boostedcreature':
		...
		foreach ($boostDB as $Creatureboost) {
		$boostDB = $SQL->query("select * from " . $SQL->tableName('boosted_boss'))->fetchAll();
		foreach ($boostDB as $Bossboost) {
		};
		die(json_encode([
	...
	case 'login':
		...
		$players = $SQL->query("select {$columns} from players where account_id = " . $account->getId() . " AND deleted = 0 order by name asc");
